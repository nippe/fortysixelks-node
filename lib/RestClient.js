...
RestClient.prototype.doRequest = function (dataObject, cb) {
	debug('Doing request');
	debug(dataObject);
	return request(dataObject).then(parseJsonReslut).catch(console.log.call(console));
};
...
function listSmsFromDate(fromDate, cb) {
	...
	return request(restClient.generate('GET', 'SMS').addFromDate(fromDate).options)
		.then(parseJsonReslut)
		.asCallback(cb)
		.catch(console.log.call(console));
}
...
function deallcoateNumber(numberId, cb) {
	return this.doRequest(this.generate('POST', '/Numbers/' + numberId, { active: 'no' })).asCallback(
		cb
	);
}
...
function setSmsUrl(numberId, smsUrl, cb) {
	...
	return this.doRequest(
		this.generate('POST', '/Numbers/' + numberId, { sms_url: smsUrl })
	).asCallback(cb);
}
...
function listCallsFromDate(fromDate, cb) {
	...
	return request(restClient.generate('GET', 'Calls').addFromDate(fromDate).options)
		.then(parseJsonReslut)
		.asCallback(cb)
		.catch(console.log.call(console));
}
...
function removeSubaccount(accountId, cb) {
	var formData = { name: name };
	return this.doRequest(this.generate('DELETE', '/Subaccounts/' + accountId).options).asCallback(
		cb
	);
}
